...
class HybridSkinModel(nn.Module):
    def __init__(self, num_classes=8, d_model=512, nhead=8, num_layers=4):
        ...
        # [CẢI TIẾN CHỐNG OVERFITTING 1]: Nâng dropout từ 0.3 lên 0.4 trong các lớp tự chú ý
        ...
        # [CẢI TIẾN CHỐNG OVERFITTING 2]: Tăng dropout tầng phân loại cuối từ 0.3 lên 0.4
        self.fc = nn.Sequential(
...
if __name__ == "__main__":
    ...
    # [CẢI TIẾN CHỐNG OVERFITTING 3]: Tăng cường Data Augmentation mạnh mẽ 
    # Tích hợp thêm Tịnh tiến hình học (RandomAffine) và Xóa ô ngẫu nhiên chống học vẹt (RandomErasing)
    ...
    # [CẢI TIẾN CHỐNG OVERFITTING 4]: Làm mịn nhãn y tế (Label Smoothing = 0.1)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    
    # [CẢI TIẾN CHỐNG OVERFITTING 5]: Nâng tham số Weight Decay từ 0.05 lên 0.10 để phạt L2 mạnh hơn
    optimizer = optim.AdamW(model.parameters(), lr=LR, weight_decay=0.10) 
    scaler = torch.amp.GradScaler('cuda') # Mixed Precision tối ưu bộ nhớ GPU

    # Lịch sử lưu thông số vẽ đồ thị đường cong học tập
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}
    
    # [CẢI TIẾN CHỐNG OVERFITTING 6]: Thiết lập Bộ kiểm soát Early Stopping dựa trên Val Loss thực chất
    best_val_loss = float('inf')
    patience = 5          # Dừng cuộc chơi nếu Val Loss tăng liên tiếp 5 Epochs
    patience_counter = 0
    ...
    for epoch in range(EPOCHS):
        ...
        # In kết quả Epoch ra màn hình và đồng bộ vào log file
        ...
        plt.tight_layout()
        plt.savefig(PLOT_SAVE_PATH, dpi=150) # Lưu đè cập nhật đồ thị thời gian thực
        plt.close()

        # -----------------------------------------------------------------
        # LOGIC GÁC CỔNG VÀ ĐÓNG BĂNG CHECKPOINT THEO ĐƯỜNG VAL LOSS
        # -----------------------------------------------------------------
        if epoch_val_loss < best_val_loss:
            best_val_loss = epoch_val_loss
            torch.save(model.state_dict(), MODEL_SAVE_PATH)
            print(f"⭐ VỊ TRÍ VÀNG! Đã đóng băng tệp trọng số tối ưu nhất tại Epoch {epoch+1} với Min Val Loss: {best_val_loss:.4f} | Val Acc: {epoch_val_acc:.2f}%")
            patience_counter = 0  # Reset bộ đếm nếu tìm được điểm hội tụ tốt hơn
        else:
            patience_counter += 1
            print(f"⚠️ Mô hình có dấu hiệu chớm Quá khớp (Overfitting) lượt thứ {patience_counter}/{patience}")

        if patience_counter >= patience:
            print(f"\n🛑 KÍCH HOẠT EARLY STOPPING TẠI EPOCH {epoch+1}!")
            print(f"👉 Hệ thống tự động đóng băng tiến trình để bảo vệ năng lực tổng quát hóa cao nhất.")
            break
